...
`promise` now lives in its own repository, [osouthgate/promise](https://github.com/osouthgate/promise). This marketplace still lists it, so `promise@content-skills` keeps working; new installs can use `claude plugin marketplace add osouthgate/promise` directly. Codex / Cursor: see [its README](https://github.com/osouthgate/promise#codex-and-other-hosts). The other plugins are written for Claude Code.
...
- `promise` - Write the promise before the build, then prove it kept. You write the outcome and rules in your own words; the model may argue in notes but never rewrite them; your acceptance rows become failing tests before any code is written. Lives in [osouthgate/promise](https://github.com/osouthgate/promise).
