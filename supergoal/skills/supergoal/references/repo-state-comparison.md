...
## The implementation: `scripts/repo-state.{sh,ps1}`
...
the repo or index. It ships in **two parity forms** with identical subcommands, arguments,
stdout, and exit codes:

- `repo-state.sh` — POSIX/bash (macOS, Linux, Codex, git-bash, WSL)
- `repo-state.ps1` — PowerShell (Windows, where the platform `bash.exe` may be a broken WSL stub)

At Stage 7 **both** are copied next to `PROTOCOL.md` into `.supergoal/`, so the `/goal` session
invokes whichever matches its host:

- Unix / git-bash: `bash .supergoal/repo-state.sh <subcommand> <args>`
- Windows / PowerShell: `pwsh -NoProfile -File .supergoal/repo-state.ps1 <subcommand> <args>`

The bash form is shown as canonical throughout the docs; substitute the `.ps1` form on Windows.
Behaviour is verified to match by `tests/repo-state.test.sh` (bash) and `tests/repo-state.test.ps1`
(PowerShell), which run the same fixture scenarios against each port.
