...
Every phase's VERIFY block reports three grep-based counts against the complete set of added/new lines since the baseline — `bash .supergoal/repo-state.sh added-lines <Baseline ref>` on Unix, or `pwsh -NoProfile -File .supergoal/repo-state.ps1 added-lines <Baseline ref>` on Windows (includes uncommitted + untracked work, not just committed; see `repo-state-comparison.md`):
