...
**Each phase keeps its full transcript contract regardless of ordering.** When phases run as subagents, each subagent prints its `SUPERGOAL_PHASE_START` / `VERIFY` / `DONE` blocks and reports its conclusion back; the orchestrator surfaces those blocks into the **main** transcript so the evaluator still sees one `SUPERGOAL_PHASE_DONE` per phase. `STATE.md` remains the single source of truth and the final audit is unchanged. Full strategy: `references/claude-capabilities.md`.
