...
**Scenarios:** 4 acceptance rows (§6), 2 worked examples (§3).
...
A second, malformed acceptance-shaped table sits beside the valid one — its
header names `Then` twice, so it does not qualify:

| # | Given | When | Then | Then |
|---|-------|------|------|------|
| AT-4 | Ana leaves the workspace | her session expires | she is signed out everywhere | she is signed out everywhere |
