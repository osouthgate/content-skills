#!/usr/bin/env python3
"""The one reader of this example's capability map (map.json).
...
and `map.checks` in its promise config. The promise plugin ships no map
tooling of its own; it only calls commands like this one, with the query or
the row id appended as exactly one argv element. Python 3 stdlib only.
...
file, that doc (relative to the map's own directory) must carry a §6 line
...
read. An optional `parent` must be a row id.
"""
...
def check(rows: Any, base_dir: str = HERE) -> List[str]:
    ...
    ``base_dir`` is the map's own directory: a row's ``doc`` is resolved
    against it, and the doc must cite the row in a §6 ``Row`` cell.
    """
    problems: List[str] = []
...
def main(argv=None) -> int:
    ...
    if args.check:
        problems = check(rows, os.path.dirname(os.path.abspath(args.map)))
