...
| "The rules are slow to confirm — I'll offer a pick-list instead." | Rules are the one thing never slated — framework § The contract, rule 7. |
...
| "This project has no detected docs home, I'll create one." | Phase 0 resolves `docsHome` from config or detection; when neither finds one it's `null` and the mode asks before writing. The skill never creates a docs folder unprompted — only on the user's explicit choice, a folder named in `new` or pinned by `adopt`, and only `render_outcome.py --create-docs-home` makes it; one you invent is a sibling-doc problem you authored. |
