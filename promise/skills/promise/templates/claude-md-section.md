<!-- promise:begin (managed by the promise skill; run /promise adopt to refresh it) -->
## Capability work goes through `/promise`

- Say what you want, in plain words. The mode is inferred; a mode word is optional.
- One doc per capability lives in {docsHome}, in the Outcome Framework shape
  ({frameworkPath}).
- §0 is the human's verbatim block. An agent only appends numbered notes below it.
- `Status:` reaches `agreed` and `shipped` only by a human's hand; `building` only
  after the human approves the red-test commit.
- A capability map, once configured in {configPath}, is the source of scenarios
  after a doc is `agreed`; §6 is then a snapshot nobody edits. A verdict moves only
  when evidence reaches the altitude the row's own `Then` claims.
- Before writing any design, plan or spec doc by hand, run `/promise` first — a
  sibling doc written outside it is a bug.
<!-- promise:end -->
